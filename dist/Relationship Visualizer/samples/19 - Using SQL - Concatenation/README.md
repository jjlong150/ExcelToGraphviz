# Using SQL – Concatenating Records into a Single Entry

Trouble viewing this document? See the online version at 
[https://exceltographviz/sql/concatenation](https://exceltographviz/sql/concatenation)

## Introduction

Sample 16 used **iteration** to determine the unique years in a dataset, and then iterated through those years to execute a second SQL statement that generated a subgroup for each year. **Concatenation** extends this pattern by allowing the iterated values to be combined into a single string that can be used directly as a node label. Instead of emitting one row per item, you can aggregate all related values—such as countries in a continent, members of a team, or songs on a playlist—into a single, well‑structured label.

This example (Sample #19) demonstrates how to add concatenation to an iteration query so that grouped data can be presented as a unified entry in your diagram.
