...
Public Sub RunSQL()
    ...
    For sqlRow = context.sqlLayout.firstRow To lastRow
        filePath = GetExcelFilePath(sqlRow, context.sqlLayout)
...
Private Function GetExcelFilePath(ByVal sqlRow As Long, ByRef sqlLayout As sqlWorksheet) As String
    Dim filePath As String
...
#If Mac Then
    ' ADODB is not available on the Mac
    executeSQL = GetMessage("msgboxSqlStatusFailure")
#Else
    ...
    For Each fieldObject In recordSetObject.fields
        ...
        Case context.headings.label
            If HasField(recordSetObject, context.fields.splitLength) Then
                ...
                DataSheet.Cells.Item(row, context.dataLayout.labelColumn) = SplitText(CStr(fieldObjectValue), splitLength, lineEnding)
            Else
