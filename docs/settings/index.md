...
![Screenshot of the Launchpad ribbon tab highlighting the Settings button.](./launchpad-settings.png)
...
This worksheet stores the six web resources that appear as clickable links on the [Source](../source/#the-source-ribbon-tab) ribbon tab. The default URLs are suggestions provided by the Relationship Visualizer’s author, but you are free to replace them with your own preferred resources. Updating the links here ensures that the corresponding buttons on the `Source` tab always open the destinations you choose.
