...
**What to do:** If a node's appearance changes after upgrading, add `shape=plaintext` explicitly, either in a style definition or a row-level  style attribute.
...
**What to do:** The `!` indicator in column 1 of the `data` worksheet is still there to flag rows with a detected error. Check the `console` worksheet for the messages. Reminder, the Console ribbon tab allows you to set your notification and message-box preferences for validation errors going forward.

## Internal cleanup, no visible effect

I tightened ribbon callback functions from public to private scope so they no longer clutter Excel's Assign Macro dialog, and removed leftover RubberduckVBA linter-suppression comments. Neither change affects how the ribbon or any feature behaves. I've only listed the change here for completeness.

## Progress bar code removed

The progress bar shown for long-running operations such as preview image generation, and large dropdown list refreshes in older versions was turned off back in v10.3 and left dormant since. I've now removed the unused code.

## Your Exchange files are not breaking

If you use the workbook Exchange feature to version-control styles and settings, older export files, including ones using the pre-v11.0 Suffix-based cluster naming, still import correctly. Everything new in v11.0 round-trips automatically going forward.
