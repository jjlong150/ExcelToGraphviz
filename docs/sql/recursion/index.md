...
- `MAX DEPTH` - Specifies an **optional** integer value which specifies the maximum number of branches preceding, or following the node specified by `WHERE VALUE`. For example, `3 AS [MAX DEPTH]` says to constrain the search to 3 edge ranks which follow a node ([Use Case 3](./#use-case-3---forward-search-from-a-node-with-max-depth)), or 3 edge ranks which precede a node ([Use Case 4](./#use-case-4---backward-search-from-a-node)), depending upon the direction of the search.
